...
export const assignSupervisor = asyncHandler(async(req, res, next)=>{
    ...
    const currentlyAssigned = supervisor.assignedStudents?.length || 0;
    if (currentlyAssigned + project.students.length > supervisor.maxStudents) {
        return next(new ErrorHandler(`Supervisor only has ${supervisor.maxStudents - currentlyAssigned} slots left, but this group has ${project.students.length} students.`, 400));
    }
